...
AppGlobal.modules.add("UserAgent", UserAgent);
...
function UserAgent_parse(userAgent,      // @arg String = navigator.userAgent
  ...
  let nav               = GlobalObject["navigator"] || {};
  let ua                = userAgent || nav["userAgent"] || "";
  let carrier           = _detectFeaturePhoneCarrier(ua);
  let os                = _detectOS(ua);
  let osVersion         = _detectOSVersion(ua, os);
  let browser           = _detectBrowser(ua);
  let browserVersion    = _detectBrowserVersion(ua, browser);
  let baseBrowser       = _detectBaseBrowser(browser, parseFloat(osVersion));
  let device            = _detectDevice(ua, os, osVersion, options, carrier);
  let lang              = _detectLanguage(nav);
  let mobile            = carrier ? true : (/Android|iOS/.test(os) || /Windows Phone/.test(ua));
  let iOS               = os === "iOS";
  let android           = os === "Android";
  let webView           = _isWebView(ua, os, browser, browserVersion, options);
  let touch3D           = /^(iPhone 6s|iPhone 7)/.test(device) ||
                          parseFloat(osVersion) >= 10 && /^iPad Pro/.test(device);
  let es5               = /native/.test(Object["keys"] + "");
  let es6               = /native/.test(String["raw"]  + "");
...
function _detectDevice(ua, os, osVersion, options, carrier) {
  let screen        = GlobalObject["screen"] || {};
  let screenWidth   = screen["width"]  || 0;
  let screenHeight  = screen["height"] || 0;
  let dpr           = options["DISPLAY_DPR"]   || GlobalObject["devicePixelRatio"] || 1.0;
  let long_         = options["DISPLAY_LONG"]  || Math.max(screenWidth, screenHeight);
  let short_        = options["DISPLAY_SHORT"] || Math.min(screenWidth, screenHeight);
  let retina        = dpr >= 2;
  let longEdge      = Math.max(long_, short_); // iPhone 4s: 480, iPhone 5: 568
...
function _getAndroidDevice(ua, retina) {
  ...
  try {
    let result = ua.split("Build/")[0].split(";").slice(-1).join().trim().
...
function _getiOSDevice(ua, retina, longEdge, osVersion) {
  ...
  let glVersion  = WebGLDetector["WEBGL_VERSION"] || "";
  let glRenderer = WebGLDetector["WEBGL_RENDERER"] || "";
//let SGX535     = /535/.test(glVersion);         // iPhone 3GS, iPhone 4
  let SGX543     = /543/.test(glVersion);         // iPhone 4s/5/5c, iPad 2/3, iPad mini
  let SGX554     = /554/.test(glVersion);         // iPad 4
  let A7         = /A7 /.test(glVersion);         // iPhone 5s, iPad mini 2/3, iPad Air
  let A8X        = /A8X/.test(glVersion);         // A8X: iPad Air 2
  let A8         = /A8 /.test(glVersion);         // A8:  iPhone 6/6+, iPad mini 4, iPod touch 6
  let A9X        = /A9X/.test(glVersion);         // A9X: iPad Pro, iPad Pro 9.7
  let A9         = /A9 /.test(glVersion);         // A9:  iPhone 6s/6s+/SE, iPad 5
  let Metal      = /Metal/.test(glVersion);       // A10: iPhone 7/7+
  let simulator  = /Software/.test(glRenderer);   // Simulator: "Apple Software Renderer"
